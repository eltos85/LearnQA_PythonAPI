...
class BaseCase:
    ...
    def get_random_name(self, lens):
        letters = string.ascii_lowercase
        name = ''.join(random.choice(letters) for i in range(lens))
        mail = ''.join(random.choice(letters) for i in range(6)) + '@example.com'
        return {
            'password': '123',
            'username': name,
            'firstName': 'learnqa',
